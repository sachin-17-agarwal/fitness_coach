...
-- Ten sessions have a blank type and two read 'Unknown'. `type` is NOT NULL,
-- so both become the one word 'Unknown' here; the hygiene step (cleanup.py,
-- step "hygiene") then infers the real type from the sets logged that day
-- and treats 'Unknown' as blank.
--
...
UPDATE workout_sessions SET type = 'Unknown' WHERE btrim(type) = '' OR lower(type) = 'unknown';
...
SELECT id, date, status FROM workout_sessions WHERE type = 'Unknown' ORDER BY date;
