...
| F4 | Pre-flight phase 2: findings that need a decision become Home decision cards; unanswered → programme default, stated | M | first cut in PR 26 Sep, unverified — the ladder question (C19) is the first kind; the nightly dry run writes it as a proposed `Ladder:` row, Home shows it with the existing card, the card's reason states the programme default | Next kinds, not built: a below-range lift's cut, a rollout's next rung, a stalled lift's lever — each needs a grammar the answer can be applied through. |
...
| S7 | Swift tests for the block review card decoding, the widget payload, the pending-reply resume | S | in PR 26 Sep, unverified | `PayloadDecodingTests` already covered the block review card, the phase split and the pending-reply classification (25 Sep); the widget payload was not testable from VauxTests because it lived in the widget target — moved to `Shared/WidgetPayload.swift`, decode test added. Runs in CI once P7 lands. Swift compile: assumed. |
...
| C15 | **Big-step machines stretch the rep range before the load moves** (athlete, 25 Sep: "what if the machine doesn't have that step"). When a lift's smallest step is more than 6% of its load, the top of the range moves up to the rep count at which one step lands back near the bottom of the range with a rep to spare (Epley), at most 6 reps over the standard top; the card shows the stretched range and the load moves only when it is reached. Past the cap the implement (microplates, an in-between dumbbell, another machine) is deferred to the coach. `prescribe.stretched_top`, used by the prescription, the READY TO LOAD watch and preflight's range check | S | merged 25 Sep (#335), unverified | Measured (export, top sets June–23 Sep): Hammer Curl 2kg on 20kg (10%), 3 of 3 increases landed under the range; Reverse Cable Fly 2.5kg on 15kg (17%), 2 of 4; Tricep Pushdown 2.5kg on 40kg (6%), 2 of 4; lifts with steps under 6% missed 4 of 34. Verify: the hammer curl's next increase lands inside 8–12. |
