...
| C12 | **Effort input revised** (athlete, 25 Sep: "7-8-9 is very hard to distinguish"): the RPE slider goes; the top set asks one of three — **Easy** (3+ left) · **On target** (1–2 left) · **Maxed** (0 left / failed) — default **Not sure** (stored unknown). Back-offs are not asked. The card says "leave 1–2 in the tank", not "@ RPE 8"; the wave's 8-vs-9 stays inside the programme (reps or the step in the peak week), never as something to feel. The scorecard judges by reps against the range; the bucket only breaks ties | S | proposed 25 Sep, saved for review | Measured: 260 of 312 September working sets (83%) carry the prefilled RPE; when the slider moved it went up 40 of 52 times, on sets at/over target reps — the obvious cases were reported, the rest left. Self-rated RPE is accurate within ±1 only near failure (Helms; Zourdos): three buckets is the honest resolution. |
