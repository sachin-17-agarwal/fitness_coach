...
| C12 | **RPE input revised (athlete, 25 Sep: "I struggle to know what each RPE felt like")**: the slider goes; the top set asks "How many more could you have done?" — 0 · 1 · 2 · 3 · 4+ · not sure — defaulting to *not sure* (stored unknown); the app converts RIR to RPE; back-offs are not asked; the card says "leave 2 in the tank" instead of "@ RPE 8"; the scorecard judges by reps alone when RIR is unknown | S | proposed 25 Sep, saved for review | Measured: 260 of 312 September working sets (83%) carry an RPE equal to the prefill. Self-rated RPE is accurate only in the last 1–2 reps (Helms; Zourdos), which is where the targets sit — RIR is the answerable form of the question. |
