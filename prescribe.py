...
def _one_step_down(load, grid, bodyweight: bool) -> float | None:
    """The load one step under `load` for a Home cut card: the lift's own
    step on a stack, one plate off the added load on a bodyweight movement
    (never below bodyweight). None when there is no load to cut."""
    if load is None:
        return None
    if bodyweight:
        return max(0.0, round(float(load) - BODYWEIGHT_INCREMENT, 2))
    if float(load) <= 0:
        return None
    step = grid or _LOAD_GRID
    return _round_load(max(0.0, float(load) - step), grid, float(load))


def _fmt_added(load, bodyweight: bool) -> str:
    if bodyweight:
        return f"BW + {float(load or 0):g}kg" if load else "bodyweight"
    return f"{float(load):g}kg"


def _unloadable_top(exercise: str, prior: "PriorSet", low: int, high: int, targets: dict, grid,
                    reasons: list, deferred: list, week_one: bool = False, asks: list | None = None) -> "SetSpec":
    ...
    if prior.reps is not None and prior.reps >= high and _is_rollout(exercise):
        ...
        if asks is not None and "standing" not in exercise.lower():
            # The one rung that is a new exercise name; farther stops are the same lift.
            asks.append({"kind": "rung", "exercise": exercise, "to": "Standing Ab Wheel Rollout",
                         "why": f"{prior.reps} reps at the top of {low}-{high} kneeling; the rollout progresses by lever, not reps"})
...
# Added load on a bodyweight movement is a plate on a dip belt or a dumbbell
# between the feet, and neither comes in kilo steps. The isolation increment
# gave a hanging leg raise +1kg a session — a load that does not exist in the
# gym — and the athlete sat at +5kg while the programme kept asking for six.
BODYWEIGHT_INCREMENT = 2.5

# A top set that has sat at the same load AND the same reps for this many
# sessions is a stall the prescription itself has to answer, not only a line
# in the readout. Same horizon as progression.DEFAULT_MIN_SESSIONS: an
# exercise recurs about 1.5x a week, so three sessions is a fortnight.
STALL_SESSIONS = 3
...
class PriorSet:
    ...
    # The watch's READY TO LOAD: a session at this load reached the top of
    # the range at RPE 9 or under (progression._programme_due). The last
    # session alone may not show it (11 of 8-12 at RPE 9.5 the day after
...
class Proposal:
    ...
    # F4 (26 Sep 2026): a deferral the athlete can answer on Home, as data —
    # {"kind": "cut", "exercise", "to", "why"} or {"kind": "rung", ...}. The
    # nightly pre-flight turns each into a proposed decision row; unanswered,
    # the programme's default stands and the card says so.
...
def _sized_overshoot(prior, load, low, high, kind, exercise: str = "",
                     athlete_kg: float | None = None):
    """The sized step when the prior set is far enough over, else None.

    A bodyweight movement is sized on what it actually lifted — the plate plus
    the movement's share of the athlete — and the step comes back as added
    load in belt-plate increments. Without a weigh-in on record, or for a
    movement that lifts no meaningful share of the body (a rollout), the
    single increment stands: sizing a step off +5kg alone would call a 15-rep
    leg raise a five-kilo lift and step it by a fraction of a plate.
    """
    if prior.reps is None or prior.reps - high < OVERSHOOT_REPS:
        return None
    if prior.bodyweight:
        return _bodyweight_overshoot(prior, load or 0.0, low, high, exercise, athlete_kg)
    if not load or load <= 0:
        return None
    return _overshoot_step(load, prior.reps, low, high, kind)


def _bodyweight_overshoot(prior, added, low, high, exercise, athlete_kg):
    fraction = bodyweight_fraction(exercise)
    if fraction is None or not athlete_kg or athlete_kg <= 0:
        return None
    body = float(athlete_kg) * fraction
    lifted = added + body
    e1rm = lifted * (1 + prior.reps / 30)
    mid = (low + high) / 2
    target = e1rm / (1 + mid / 30)
    raw = target - lifted
    cap = lifted * OVERSHOOT_CAP
    step = max(min(raw, cap), BODYWEIGHT_INCREMENT)
    new = _round_to(added + step, BODYWEIGHT_INCREMENT)
    new = max(new, added + BODYWEIGHT_INCREMENT)
    reason = (f"Load BW + {new:g}kg: {prior.reps} reps at BW + {added:g}kg is about "
              f"{lifted:g}kg lifted ({body:g}kg of you), which puts {mid:g} reps near "
              f"{_round_load(target):g}kg lifted"
              + ("; capped at +10% this session" if raw > cap else "")
              + f". Added load moves in {BODYWEIGHT_INCREMENT:g}kg plates. Reps above the "
              f"{low}-{high} range are a backlog, not an achievement (:205).")
    return new, reason
...
def apply_recovery(spec: "SetSpec", adjustment: RecoveryAdjustment,
    ...
    if adjustment.load_multiplier != 1.0 and weight is not None:
        ...
        if cut == weight:
            ...
            reasons.append(
                f"Readiness: effort down, load held — a {abs(1 - adjustment.load_multiplier) * 100:.0f}% cut is "
                f"under this lift's {spec.grid:g}kg step (C1)."
            )
        weight = cut
...
def next_top_set(exercise: str, kind: str, week: int, prior: PriorSet | None,
                 ...
                 athlete_kg: float | None = None,
                 rep_range: tuple | None = None, asks: list | None = None) -> SetSpec:
    ...
    if prior.reps is not None and prior.reps < low and week != deload_week():
        ...
        cut_to = _one_step_down(load, grid, bodyweight)
        if asks is not None and cut_to is not None:
            asks.append({"kind": "cut", "exercise": exercise, "to": cut_to,
                         "why": f"ran {prior.reps} against {low}-{high} at {_fmt_added(load, bodyweight)}"})
    ...
    if week == 1:
        ...
        if bodyweight and is_unloadable(exercise) and not (prior.load or 0):
            return _unloadable_top(exercise, prior, low, high, targets, grid, reasons, deferred, week_one=True, asks=asks)
    ...
    if (prior.held >= STALL_SESSIONS and prior.reps is not None
        ...
        # seven sessions at RPE 6-7 while the programme asked for 9-12 each
        # time; the coach did the coaching by hand every session. When the
        # RPE says the reps are there, the prescription pins them at the top
        ...
        if asks is not None and cut_to is not None:
            asks.append({"kind": "cut", "exercise": exercise, "to": cut_to,
                         "why": f"stalled {prior.held} sessions at {held_at.rsplit(' RPE', 1)[0]}, the last logged harder than the card"})
    ...
    if bodyweight and is_unloadable(exercise) and not (prior.load or 0) and week != deload_week():
        ...
        return _unloadable_top(exercise, prior, low, high, targets, grid, reasons, deferred, asks=asks)
    ...
    if week == 2:
        ...
        if prior.reps is not None and prior.reps > high:
            sized = _sized_overshoot(prior, load, low, high, kind, exercise, athlete_kg)
            if sized:
                load, why = sized
...
def prescribe_exercise(exercise: str, sets: int, kind: str, week: int,
    ...
    sized on what it lifts rather than on the plate alone. None is fine: the
    single increment stands.
    """
    reasons: list[str] = []
    deferred: list[str] = []
    ...
    asks: list = []
    top = next_top_set(exercise, kind, week, prior, reasons, deferred, athlete_kg, rep_range=rep_range, asks=asks)
    if (top.bodyweight and top.weight_kg and prior is not None and not prior.load
        ...
        # the programme's lever is real (a plate on the back, a vest) but it
        # is not a belt, and saying so keeps the number honest.
        reasons.append(
            f"Added load on {exercise} means a plate on the back or a vest. If that "
    ...
    return Proposal(
        ...
        reasons=reasons, deferred=deferred, straight=straight, asks=asks,
    )
